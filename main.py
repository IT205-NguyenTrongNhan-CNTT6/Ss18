...
def show_inventory(inventory):
    ...
    for items in inventory:
        print(f"{items.get('id'):<12}| {items.get('name'):<17}| {items.get('quanlity'):<17}")
    ...
    while True:
        ...
        for item in inventory:
            if(upd_id.lower() == item.get("id").lower()):
                print(f"Tìm thấy hàng hóa : {item.get('name')} (Số lượng hiện tại:{item.get("quanlity")})") 
            upd_quanlity = get_validate_input("Nhập số lượng làm mới: ","int")
            inventory["quanlity"] == upd_quanlity
